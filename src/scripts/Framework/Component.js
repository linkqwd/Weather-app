export default class Component {
  ...
  _render() {
    ...
    if (typeof content === 'string') {
      ...
      content.map(item => this._vDomPrototypeElementToHtmlElement(item)).forEach(htmlElement => this.host.appendChild(htmlElement));
    }
  }
      ...
      if (element.tag) {
        if (typeof element.tag === 'function') {
          ...
          ['classList', 'attributes', 'children', 'eventHandler'].forEach(item => {
            if (element[item] && !Array.isArray(element[item])) element[item] = [element[item]];
          });

          if (element.classList) container.classList.add(...element.classList);

          if (element.attributes) element.attributes.forEach(attr => container.setAttribute(attr.name, attr.value));
